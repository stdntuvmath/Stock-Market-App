...
namespace Stock_Trading_App
{
    internal static class Buy_UsingAVGSlope
    {
        ...
        private static bool AvgSlopeIsGoingDown = false;
        ...
        private static decimal currentAVGSlope = 0.00M;
        private static decimal previousAVGSlope;
        private static decimal secondPreviousAVGSlope;
        private static decimal thirdPreviousAVGSlope;
        ...
        public static async Task Buy_UsingAVGSlopeMethod(string stockListing, int numberOfShares)
        {
            try
            {
                ...
                //stock market hours
                TimeSpan start = new TimeSpan(8, 29, 45); //8:30 AM
                TimeSpan end = new TimeSpan(14, 59, 45); //3 o'clock military time
                ...
                else
                {
                    ...
                    while ((start <= now) && (now < end) && today != "Saturday" && today != "Sunday")//In production, these should have AND's "&&" not OR's "||"
                    {
                        ...
                        if (AVGSlopeArray.Count == 0)
                        {
                            //do nothing
                        }
                        else
                        {
                            currentAVGSlope = AVGSlopeArray[AVGSlopeArray.Count];
                        }
                        if (AVGSlopeArray.Count-1 == 0)
                        {
                            //do nothing
                        }
                        else
                        {
                            previousAVGSlope = AVGSlopeArray[AVGSlopeArray.Count-1];
                        }
                        if (AVGSlopeArray.Count - 2 == 0)
                        {
                            //do nothing
                        }
                        else
                        {
                            secondPreviousAVGSlope = AVGSlopeArray[AVGSlopeArray.Count - 2];
                        }
                        if (AVGSlopeArray.Count - 3 == 0)
                        {
                            //do nothing
                        }
                        else
                        {
                            thirdPreviousAVGSlope = AVGSlopeArray[AVGSlopeArray.Count - 3];
                        }
                        equity = decimal.Zero;
                        ...
                        //{
                        //    previousAVGSlope = AVGSlopeArray[AVGSlopeArray.Count - 2];

                        //}

                        now = DateTime.Now.TimeOfDay;
                        logFileLine = now + "|" + "currentAVGSlope: " + currentAVGSlope+"\rpreviousAVGSlope: "+ previousAVGSlope+"\r\r";
                        File.AppendAllText(logFilePath, logFileLine);
                        ...
                        else if ((previousAVGSlope < currentAVGSlope && equity > buyInEquity || equity <= maxEquity - 1.00M) && OKtoBuy == false)//this one worked the best for selling - please don't delete!!
                        ...
                        else if ((previousAVGSlope > currentAVGSlope) && !(secondPreviousAVGSlope < previousAVGSlope) && OKtoBuy == true)//this is where we buy - 
                        {
                            equity = decimal.Zero;
                            account = await client.GetAccountAsync();
                            ...
                            logFileLine = "************************\r" + now + "|" + "AUTOBUY " + 
                                            "\r" + "buyPrice: " + buyPrice.ToString() + 
                                            "\r" + "buyInEquity: " + buyInEquity.ToString() +
                                            "\r" + "previousAVGSlope > currentAVGSlope:\r" + previousAVGSlope.ToString() +"\r"+ currentAVGSlope.ToString()+
